...
namespace MProject.Animation {
    public class SceneLinkedSMB<T> : SealedSMB where T : MonoBehaviour {
        ...
        public static void Initialize(Animator _animator, T _mono_behaviour) {
            SceneLinkedSMB<T>[] sceneLinkedSMBs = _animator.GetBehaviours<SceneLinkedSMB<T>>();
            for (int i = 0; i < sceneLinkedSMBs.Length; i++) {
                sceneLinkedSMBs[i].InternalInitialize(_animator, _mono_behaviour);
            }
        }

        protected void InternalInitialize(Animator _animator, T _mono_behaviour) {
    ...
    public abstract class SealedSMB : StateMachineBehaviour {
        ...
        public sealed override void OnStateMove(Animator _animator, AnimatorStateInfo _state_info, int _layer_index) {
        }
        public sealed override void OnStateIK(Animator _animator, AnimatorStateInfo _state_info, int _layer_index) {
        }
